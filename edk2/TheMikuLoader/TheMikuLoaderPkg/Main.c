...
EFI_STATUS EFIAPI UefiMain(EFI_HANDLE image_handle,EFI_SYSTEM_TABLE* system_table){
    ...
    Print(L"Frame Buffer: 0x%0lx - 0x%0lz, Size: %lu bytes\n",
        gop->Mode->FrameBufferBase, // 先頭アドレス
        gop->Mode->FrameBufferBase + gop->Mode->FrameBufferSize, // 全体サイズ
    ...
    struct FrameBufferConfig config = {
        ...
        gop->Mode->Info->PixelsPerScanLine,
        gop->Mode->Info->HorizontalResolution, // Resolution = 解決策 = 解像度
        gop->Mode->Info->VerticalResolution,
        0
    };
